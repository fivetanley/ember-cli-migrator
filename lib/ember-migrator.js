...
EmberMigrator.prototype.splitFile = function(filePath) {
  ...
  var addNodeSomewhere = function(node, filePath, className) {
    ...
    var fileName = filePathForClassname(className, newType, filePath);
...
EmberMigrator.prototype.preProcessFile = function(filePath, ast) {
  ...
  if (type === 'model') {
    ...
    this.classesFilepathMap[exportName] = '/' +  this.appName + '/mixins/' + fileName;
  } else if (type === 'controller') {
    this.classesFilepathMap[exportName] = '/' +  this.appName + '/controllers/' + fileName;
  } else if (type === 'view') {
    this.classesFilepathMap[exportName] = '/' +  this.appName + '/views/' + fileName;
  } else {
...
EmberMigrator.prototype.processFile = function(filePath, ast){
  ...
  var type = determineType(path.dirname(filePath), filePath);
  var fileName = path.basename(filePath);
...
function determineType(filePath, className) {
  ...
  if (/page_mixin/.test(filePath)) {
    console.log('\n\n');
    console.log('hooray', filePath, 'class', className);
    console.log('\n\n');
  }
  if (SERIALIZER.test(className)) {
    return 'serializer';
  } else if (VIEW.test(className)) {
    return 'view';
  } else if (MIXIN.test(className)) {
    return 'mixin';
  } else if (CONTROLLER.test(className)) {
    return 'controller';
  } else if (MODEL_PATH.test(filePath)) {
    return 'model';
  } else if (VIEWS_PATH.test(filePath)) {
    return 'view';
  } else if (CONTROLLERS_PATH.test(filePath)) {
    return 'controller';
  } else if (MIXINS_PATH.test(filePath)) {
    return 'mixin';
...
function filePathForClassname(className, type, filePath) {
  if (type === 'unknown') {
    return filePath;
  }
  ...
  if (type === 'serializer' && /serializer/.test(fileParts[fileParts.length-1])) {
    fileParts.pop();
  } else if (type === 'view' && /view/.test(fileParts[fileParts.length-1])) {
    fileParts.pop();
  } else if (type === 'controller' && /controller/.test(fileParts[fileParts.length-1])) {
    fileParts.pop();
  }  else if (type === 'mixin' && /mixin/.test(fileParts[fileParts.length-1])) {
    fileParts.pop();
  }
