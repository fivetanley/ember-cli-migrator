...
EmberMigrator.prototype.run = function EmberMigrator_run(){
  ...
  var copyFile = function (destSubDir, file) {
    var fullPath = path.join(self.inputDirectory, file);
    var outputFile;
    if (destSubDir) {
      var destFileWithSubDir = path.join(destSubDir, path.basename(file));
      outputFile = path.join(self.outputDirectory, self.appName, destFileWithSubDir);
    } else {
      outputFile = path.join(self.outputDirectory, self.appName, 'nonjs', file);
    }
  ...
  hbsFiles.forEach(copyFile.bind(this, 'templates'));
  nonJsFiles.forEach(copyFile.bind(this, null));
