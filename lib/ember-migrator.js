...
EmberMigrator.prototype.preProcessFile = function(filePath, typedExport) {
  ...
  // TODO(Tony) what is the difference bewteen this filePath and one originally stored?
  this.classesFilepathMap[typedExport.exportName] = typedExport.filePathForType(typedExport.type, this.appName, filePath);
};

EmberMigrator.prototype.processFile = function(filePath, typedExport){
  //TODO(Igor) split properly into fileName and className
  var type = typedExport.determineType(path.dirname(filePath), filePath);
  ...
  this.convertedFiles[type][fileName] = this.convertFile(fileName, filePath, typedExport);
};

EmberMigrator.prototype.convertFile = function(modelName, filePath, typedExport){
