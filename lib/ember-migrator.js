...
EmberMigrator.prototype.splitFile = function(filePath) {
  ...
  var addTypedNode = function(node, filePath, className, isWindow) {
    ...
    if (that.splitFiles[fileName] && className) {
      ...
      var nameWithoutExt = fileName.slice(0,fileName.indexOf(ext));
      fileName = nameWithoutExt + '-x' + ext;
      // TODO(Tony) Not sure why this would happen again, so just going to fail
      // here rather than continuously append Xs.
      if (that.splitFiles[fileName]) {
        console.error('Bad things happening, multiple redundant global export for className:', className);
      }
    }
    if (!that.splitFiles[fileName]) {
