...
EmberMigrator.prototype.run = function EmberMigrator_run(){
  ...
  // Copy all non-js files to a non-js directory in the new app
  var otherFiles = walk(this.inputDirectory);
  otherFiles = otherFiles.filter(function(file){
    var fullPath = path.join(self.inputDirectory, file);
    var isDir = fs.lstatSync(fullPath).isDirectory();
    var isNonJS = !string.endsWith(file, '.js');
    return !isDir && isNonJS;
  });

  otherFiles.forEach(function (file) {
    var fullPath = path.join(self.inputDirectory, file);
    var outputFile = path.join(self.outputDirectory, self.appName, 'nonjs', file);
    var outputFolder = path.dirname(outputFile);
    console.log('moving nonjs file', fullPath, 'to', outputFile);
